...
const GAG_REGEN_REFERENCE := preload("res://objects/battle/battle_resources/status_effects/resources/status_effect_gag_discount.tres")
	...
	if movie_type == MovieType.LADDER:
		...
		if status_effect:
			...
			if movie_type == MovieType.PIXIE:
				new_effect.amount = ceili(target.stats.max_hp * (0.2 + target.stats.toonup_boost))
				new_effect.description = "%.d%% Laff Regeneration" % ((0.2 + target.stats.toonup_boost) * 100)
				BattleService.ongoing_battle.affect_target(target, -new_effect.amount)
			elif new_effect is StatBoost:
...
## Gets properly registered version of gag regen boost
func get_gag_regen(base_effect : StatusEffectGagRegen) -> StatusEffect:
	var new_effect := GAG_REGEN_REFERENCE.duplicate()
...
func get_status_effect_copy(base_effect: StatusEffect) -> StatusEffect:
	...
	elif base_effect is StatusEffectGagRegen:
		return get_gag_regen(base_effect)
	return base_effect.duplicate()
